...
function makeProspectFolder(prospectId, propertyAddress) {
  const safeProspectId = makeSafePublicId(prospectId) || "motion-test";
  const safeStreetAddress = makeSafeStreetAddress(propertyAddress);
  return safeStreetAddress ? `${safeProspectId}_${safeStreetAddress}` : safeProspectId;
}
...
function validateMotion({ start_zoom, end_zoom, start_x, end_x, start_y, end_y, duration, fps, easing }) {
  const numbers = { start_zoom, end_zoom, start_x, end_x, start_y, end_y, duration, fps };
  ...
  if (fps <= 0) return "fps must be greater than 0";
  if (start_zoom <= 0 || end_zoom <= 0) return "start_zoom and end_zoom must be greater than 0";

  for (const [name, value] of Object.entries({ start_x, end_x, start_y, end_y })) {
    if (value < 0 || value > 1) return `${name} must be between 0 and 1`;
  }
...
function buildMotionFilter({ start_zoom, end_zoom, start_x, end_x, start_y, end_y, duration, fps, easing }) {
  const frames = Math.max(2, Math.round(duration * fps));
  const lastFrame = frames - 1;
  const progress = easing === "smoothstep"
    ? `(3*pow(min(on/${lastFrame},1),2)-2*pow(min(on/${lastFrame},1),3))`
    : `min(on/${lastFrame},1)`;

  const zoomDelta = end_zoom - start_zoom;
...
async function renderMotionClip(params) {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), "ssp-rnd-"));
  const inputPath = path.join(workDir, "input.jpg");
  const outputPath = path.join(workDir, "motion.mp4");

  try {
    await downloadImage(params.image_url, inputPath);
    const filter = buildMotionFilter(params);
    ...
    const folder = makeProspectFolder(params.prospect_id, params.property_address);
    const outputName = makeSafePublicId(params.output_name || "custom-motion") || "custom-motion";
    const key = `ssp-prospects/${folder}/tests/${outputName}.mp4`;
    return await uploadVideo(outputPath, key);
  } finally {
...
app.post("/test-cinematic-motion", async (req, res) => {
  const presets = {
  ...
  if (!presets[preset]) return res.status(400).json({ success: false, error: `Unknown preset: ${preset}` });
  ...
  const error = validateMotion(params);
...
app.post("/test-cinematic-motion-custom", async (req, res) => {
  const {
    image_url,
  ...
  const params = {
    ...
    fps: Number(fps),
    easing,
    start_zoom: Number(start_zoom),
    end_zoom: Number(end_zoom),
  ...
  try {
    ...
    return res.json({
      ...
      public_id: upload.key,
      render: {
        route: "/test-cinematic-motion-custom",
        output_name,
        duration: params.duration,
        fps: params.fps,
        easing,
    ...
    console.error("[RND CUSTOM] Render failed:", error.stderr || error.message || error);
    return res.status(500).json({ success: false, error: error.message || "Custom render failed" });
  }
});

app.listen(PORT, "0.0.0.0", () => {
  console.log(`${SERVICE_NAME} v${SERVICE_VERSION} listening on port ${PORT}`);
  console.log(`[RND] Custom route: /test-cinematic-motion-custom`);
  console.log(`[S3] Region: ${AWS_REGION}`);
  console.log(`[S3] Bucket: ${AWS_S3_BUCKET || "NOT CONFIGURED"}`);
});
